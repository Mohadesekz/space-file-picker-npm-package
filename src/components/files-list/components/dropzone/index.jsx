...
class Dropzone extends React.Component {
  ...
  componentDidMount() {
    ...
    if (this.goToRow) {
      setTimeout(() => {
        const infinitScroll = document.querySelector('.infinite-scroll-component');
        Util.scrollToPosition(infinitScroll, this.goToRow.parentNode.offsetTop - 100, 300);
      }, 300);
    }
  }
  ...
  onDrop(evt) {
    ...
    if (isDragDataWithFiles(evt)) {
      Promise.resolve(getDataTransferItems(evt)).then(fileList => {
        ...
        AppToaster.dismiss('max-length-is-over');
        if (fileList.length > maxLength) {
          AppToaster.show(
            {
              message: 'حداکثر ۱۰۰۰ فایل به صورت هم زمان می توانید آپلود کنید.',
              // timeout: '0',
              icon: 'danger-sign',
  ...
  render() {
    ...
    return renderAsTr ? (
      ...
      >
        ...
        <td
          ref={el => {
            this.props.recent === 'true' ? (this.goToRow = el) : (this.goToRow = undefined);
          }}
        >
          <input
            {
...
Dropzone.propTypes = {
  ...
  /**
   * Rendering the component as tr instead of div
   */
  renderAsTr: PropTypes.bool,
